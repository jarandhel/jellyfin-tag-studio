...
- **Collections** — bulk add to and remove from collections using the same chips; typing a
  name that does not exist creates it.
...
### Collections are not a field

Tags and genres are arrays on the item; collection membership is `LinkedChildren` on a
`BoxSet` pointing the other way. So collections get their own read path - one pass over
every BoxSet's `LinkedChildren` building a reverse index, rather than testing each item
against each collection - and their own write path through `ICollectionManager`.

They also cannot ride along in the per-item journal entries, which record before/after
field values. Membership changes are journalled separately as `CollectionChange` records
and inverted on undo, so a mixed operation does not half-restore.

Collections generated by another plugin (SmartCollections names them `... [Smart]`) are
refused for the same reason machine tags are excluded from bulk delete: the edit appears
to work until the generator next runs. Configurable via `ManagedCollectionMarkers`.
...
| `ManagedCollectionMarkers` | `[Smart]` | Collections another plugin generates |
...
| GET | `/Collections` | every collection, with size and managed flag |
| POST | `/Collections/Apply` | bulk add to / remove from collections |
