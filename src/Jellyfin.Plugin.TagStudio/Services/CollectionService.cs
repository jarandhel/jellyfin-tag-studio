...
public class CollectionService
{
    ...
    /// <summary>
    /// Collections with nothing visible in them. Jellyfin's own "Clean up collections and
    /// playlists" task strips missing members out of a collection but never removes the
    /// collection itself, so these accumulate on their own.
    /// </summary>
    public EmptyCollectionReport FindEmpty()
    {
        var boxSets = LoadBoxSets();
        var paths = BuildPathIndex();
        var found = new List<EmptyCollection>();
        ...
        foreach (var boxSet in boxSets)
        {
            ...
            var name = boxSet.Name ?? string.Empty;
            var entry = new EmptyCollection
            {
                Id = boxSet.Id,
                Name = name,
                LinkedChildCount = linked,
                Reason = linked == 0 ? EmptyCollectionReason.NoMembers : EmptyCollectionReason.Unresolvable,
                IsManaged = IsManaged(name)
            };

            entry.Blocker = BlockerFor(entry, boxSet);
            entry.Deletable = entry.Blocker is null;
            found.Add(entry);
        }

        return new EmptyCollectionReport
        {
            Scanned = boxSets.Count,
            Collections = found
                .OrderByDescending(c => c.Deletable)
                .ThenBy(c => c.Name, StringComparer.OrdinalIgnoreCase)
                .ToArray(),
            DeletableCount = found.Count(c => c.Deletable)
        };
    }
    ...
    /// collection can read as empty while still being worth keeping.
    /// </summary>
    private string? BlockerFor(EmptyCollection entry, BoxSet boxSet)
    {
        if (entry.IsManaged)
        {
            return "generated by another plugin - it would simply be recreated";
        }

        if (entry.Reason == EmptyCollectionReason.Unresolvable)
        {
            // The dangerous case: an unmounted drive makes every item on it unresolvable,
            // and so makes every collection of those items look empty. Never act on this
            // automatically - report it and let a human look.
            return entry.LinkedChildCount + " member(s) recorded but none resolve; if a "
                 + "library drive is offline this collection is not really empty";
        }

        var path = boxSet.Path;
        if (!string.IsNullOrEmpty(path) && !IsUnderDataFolder(path))
        {
            // A legacy box set is backed by a real directory rather than by Jellyfin's own
            // metadata folder, and deletion takes the file location with it.
            return "stored outside Jellyfin's data folder, so deleting it would remove a real directory";
        }

        return null;
    }

    private bool IsUnderDataFolder(string path)
    {
        ...
        return Path.GetFullPath(path)
            .StartsWith(Path.GetFullPath(dataPath), StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Deletes empty collections. Deliberately not journalled: a deleted collection cannot
    /// be restored by writing a field back, so offering undo here would be a lie. Every
    /// candidate is re-checked immediately before removal in case it gained members since
    /// the scan.
    /// </summary>
    public DeleteEmptyCollectionsResult DeleteEmpty(
        DeleteEmptyCollectionsRequest request,
        CancellationToken cancellationToken)
    {
        var report = FindEmpty();
        var deletable = report.Collections.Where(c => c.Deletable).ToDictionary(c => c.Id);
        var deleted = new List<string>();
        var skipped = new List<string>();
        var warnings = new List<string>();

        List<EmptyCollection> targets;
        if (request.Ids.Length == 0)
        {
            targets = deletable.Values.ToList();
        }
        else
        {
            targets = new List<EmptyCollection>();
            foreach (var id in request.Ids)
            {
                if (deletable.TryGetValue(id, out var match))
                {
                    targets.Add(match);
                    continue;
                }

                var known = report.Collections.FirstOrDefault(c => c.Id == id);
                skipped.Add(known is null
                    ? id + " is not an empty collection"
                    : "\"" + known.Name + "\"" + " - " + known.Blocker);
            }
        }
        ...
        foreach (var target in targets)
        {
            ...
            if (_libraryManager.GetItemById(target.Id) is not BoxSet boxSet)
            {
                skipped.Add("\"" + target.Name + "\"" + " - no longer present");
                continue;
            }
            ...
            try
            {
                ...
                deleted.Add(target.Name);
                _logger.LogInformation("Deleted empty collection {Name} ({Id})", target.Name, target.Id);
            }
            catch (Exception ex)
            {
                warnings.Add("\"" + target.Name + "\"" + " could not be deleted: " + ex.Message);
                _logger.LogError(ex, "Failed to delete empty collection {Name}", target.Name);
            }
        }

        if (!request.DryRun && deleted.Count > 0)
        {
            InvalidateMembershipCache();
        }

        return new DeleteEmptyCollectionsResult
        {
