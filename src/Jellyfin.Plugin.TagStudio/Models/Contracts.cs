...
/// <summary>
/// Why a collection reads as empty. This is the difference between "nothing was ever in
/// it" and "what was in it cannot be found right now", and only the first is safe to act on.
/// </summary>
public enum EmptyCollectionReason
{
    /// <summary>No member entries at all.</summary>
    NoMembers,

    /// <summary>Member entries exist but none resolve to a library item.</summary>
    Unresolvable
}

/// <summary>One collection with no visible members, and whether it is safe to remove.</summary>
public class EmptyCollection
{
    ...
    /// <summary>Member entries recorded on the BoxSet, resolvable or not.</summary>
    public int LinkedChildCount { get; set; }

    public EmptyCollectionReason Reason { get; set; }

    /// <summary>Generated by another plugin, so deleting it would only invite it back.</summary>
    public bool IsManaged { get; set; }

    public bool Deletable { get; set; }

    /// <summary>Why this one is being left alone; null when it is deletable.</summary>
    public string? Blocker { get; set; }
}

public class EmptyCollectionReport
{
    public int Scanned { get; set; }

    public IReadOnlyList<EmptyCollection> Collections { get; set; } = Array.Empty<EmptyCollection>();

    public int DeletableCount { get; set; }
}

public class DeleteEmptyCollectionsRequest
{
    /// <summary>Which to delete. Empty means everything the scan marks deletable.</summary>
    public Guid[] Ids { get; set; } = Array.Empty<Guid>();

    /// <summary>Defaults to true so an unparameterised call cannot destroy anything.</summary>
    public bool DryRun { get; set; } = true;
}

public class DeleteEmptyCollectionsResult
{
    public bool DryRun { get; set; }

    public IReadOnlyList<string> Deleted { get; set; } = Array.Empty<string>();
