...
export interface CollectionEntry {
  ...
  name: string;
  itemCount: number;
  /** Generated by another plugin, so a manual edit would be reverted. */
  isManaged: boolean;
}
...
const mapItem = (i: any): ItemRow => ({
  ...
  collections: asArray<string>(i?.Collections),
  dateCreated: i?.DateCreated ?? '',
  tagsLocked: !!i?.TagsLocked,
  genresLocked: !!i?.GenresLocked
});

const mapCollection = (c: any): CollectionEntry => ({
  id: c?.Id ?? '',
  name: c?.Name ?? '',
...
export const api = {
  ...
  collections: () => request<any>('/Collections').then((r) => asArray<any>(r).map(mapCollection)),

  applyCollections: (body: { itemIds: string[]; addTo: string[]; removeFrom: string[] }) =>
    post<any>('/Collections/Apply', {
      ItemIds: body.itemIds,
      AddTo: body.addTo,
